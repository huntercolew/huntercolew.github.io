...
_.identity = function(value){ // return unchanged value
    return value;
};
...
_.typeOf = function(value){ // return type of value as a string
...
_.last = function(array, num){
    ...
    if(num < 0) return [];
    if(num > array.length) return array;
    for(var i = array.length - num; i <= array.length - 1; i++){
        resultArray.push(array[i]);
    }
    return resultArray;
};
...
_.unique = function(array){
    ...
    for(var i = 0; i < array.length; i++){
        if(_.indexOf(array, array[i]) == i){
            result.push(array[_.indexOf(array, array[i])]);
        }
    }
    return result;
};
...
_.filter = function(array, theFunc){
    var result = [];
    function pushArray(element, index, collection){
        if(theFunc(element, index, collection)){
            result.push(element);
        }
    }
    _.each(array, pushArray);
    return result;
};
...
_.reject = function(array, testFunc){
    ...
    return _.filter(array, testFuncBundle);
};
...
_.map = function(collection, testFunc){
    var result = [];
